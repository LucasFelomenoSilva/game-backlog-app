...
function App() {
  ...
  const playNotificationSound = () => {
    const audio = new Audio(
      "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhBSuBzvLZizcIGWi7eefTRAMUKfj+LZjHAY4ktfzznksBS",
    );
    audio.volume = 0.3;
    audio.play().catch(() => {});
  };
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        setUser({
          ...
          favorites: firestoreData.favorites || [], // <-- Puxando do Firestore!
        });
        await initUserSocialProfile(
          currentUser.uid,
        ...
        fetchedGamesData = fetchedGamesData.map((game) => {
          ...
          if (game.status === "zerados" && !game.finishedDate) {
            ...
            if (historyDate)
              return {
                ...game,
                finishedDate: new Date(historyDate + "T12:00:00").toISOString(),
              };
          }
          return game;
        });
  ...
  const handleCompleteGameFinish = (reviewData) => {
    ...
    const newGamesData = gamesData.map((game) =>
      game.id === gameToReview.id ? updatedGame : game,
    );
    setGamesData(newGamesData);
    ...
    setGameHistory((prev) => [
      ...prev,
      { game: updatedGame.nome, status: "zerado", date: today.split("T")[0] },
    ]);
    triggerConfetti();
  ...
  const groupedGames = gamesData.reduce(
    (acc, game) => {
        ...
        <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
          <div className="text-center z-10 p-6 bg-gray-900/80 backdrop-blur-xl rounded-2xl border border-cyan-500/30 shadow-2xl">
            ...
            <div className="w-48 h-2 bg-gray-700 rounded-full mx-auto mb-4 overflow-hidden">
              <div
                className="h-full bg-cyan-500 animate-[loading_2s_ease-in-out_infinite]"
                style={{ width: "50%" }}
              ></div>
            </div>
    ...
    if (isRecommenderOpen)
      return (
        <GameRecommender
          ...
          onSelectGame={handleSelectRecommendation}
        />
      );
