...
import { Joystick, Sparkles, Gamepad2, Plus, Loader2, Trophy } from "lucide-react";
import imageCompression from "browser-image-compression";
import { DragDropContext } from "@hello-pangea/dnd";
import { auth, db, googleProvider } from "./firebase";
import { onAuthStateChanged, signInWithPopup, signOut } from "firebase/auth";
import { doc, getDoc, setDoc, updateDoc, collection, query, where, getDocs } from "firebase/firestore";
import { initUserSocialProfile, subscribeToSocialProfile } from "./services/socialService";
import { useTheme } from "./context/ThemeContext";
import { useUndoDelete } from "./hooks/useUndoDelete";

// ── Lazy imports: telas de aba e modais pesados ──────────────────────────────
import {
  LazyProgressScreen,
  LazyProfileScreen,
  LazyEnhancedAchievements,
  LazyFriendsScreen,
  LazyWrappedScreen,
  LazyGameRecommender,
  LazyBackupRestore,
...
// ── Fallback de carregamento para modais ─────────────────────────────────────
const ModalLoader = () => (
...
// ── Confetti memoizado: não recria os 50 itens a cada render ─────────────────
const Confetti = React.memo(() => {
  ...
  const pieces = useMemo(
    ...
    [] // só calcula uma vez por montagem
  );
  return (
...
function App() {
  ...
  const [isPublicRoute] = useState(() => window.location.pathname.startsWith('/u/'));
  ...
  useEffect(() => {
    ...
    if (path.startsWith('/u/')) {
      ...
      else setPublicViewError("Link de utilizador inválido.");
    }
  }, []);
  ...
  const fetchPublicProfile = async (username) => {
    ...
    try {
      ...
      if (querySnapshot.empty) { setPublicViewError("Perfil não encontrado 😢"); return; }
    ...
    } catch { setPublicViewError("Erro ao carregar o perfil."); }
    ...
    if ("Notification" in window && Notification.permission !== "granted") {
      ...
      if (perm === "granted") {
        try {
          ...
          const sub = await reg.pushManager.subscribe({ userVisibleOnly: true, applicationServerKey: "SUA_CHAVE_VAPID_AQUI" });
  ...
  const handleGoogleSignIn = async () => {
    try { await signInWithPopup(auth, googleProvider); toast.success("Player 1 Connected!"); }
    catch { toast.error("Erro ao conectar controle (Login)."); }
  };

  const handleSignOut = async () => {
    try { await signOut(auth); toast.success("Player 1 Disconnected."); }
  ...
  const handleProfileImageUpload = async (file) => {
    try {
      ...
      reader.onloadend = async () => {
        ...
        if (user) { await updateDoc(doc(db, "users", user.uid), { photoBase64: base64String }); toast.success("Avatar atualizado!"); }
      };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        else { const initialData = { gamesData: [], achievements: [], gameHistory: [] }; await setDoc(userDocRef, initialData); firestoreData = initialData; }
        setUser({ ...currentUser, photoBase64: firestoreData.photoBase64 || null, favorites: firestoreData.favorites || [] });
        ...
        const historyMap = {};
        fetchedHistory.forEach((item) => { if (item.status === "zerado" && item.date) historyMap[item.game] = item.date; });
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToSocialProfile(user.uid, (profile) => {
      setSocialProfile((prev) => {
        ...
        if (newCount > prevCount) toast("📬 Novo pedido de amizade!", { icon: "👾", style: { background: "#1e3a5f", color: "#7dd3fc", border: "1px solid #0ea5e9" }, duration: 5000 });
        return profile;
      });
    });
    return () => unsub();
    ...
    async (currentGamesData, currentAchievements, currentHistory) => {
      ...
      try {
        await updateDoc(doc(db, "users", user.uid), {
          gamesData: JSON.parse(JSON.stringify(currentGamesData)),
          achievements: currentAchievements,
          gameHistory: JSON.parse(JSON.stringify(currentHistory)),
  ...
  const handleSelectRecommendation = (game) => { setDraftGame(game); setIsRecommenderOpen(false); setIsAddGameModalOpen(true); };
  ...
  const getCategoryProgress = useCallback((category) => gamesData.filter((g) => g.status === category).length, [gamesData]);
  ...
  const groupedGames = useMemo(() => gamesData.reduce((acc, game) => {
    const status = game.status;
    if (!acc[status]) acc[status] = [];
    acc[status].push(game);
    return acc;
  }, { playing: [], installed: [], backlog: [], zerados: [], desejados: [] }), [gamesData]);
  ...
  if (isPublicRoute) {
    ...
    if (publicViewError || !publicViewData) return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center" style={{ background: V.bg }}>
        ...
        <a href="/" className="px-6 py-3 rounded-2xl font-bold text-white transition-all hover:scale-105 shadow-xl" style={{ background: `linear-gradient(135deg, ${V.primary}, ${V.secondary})` }}>Criar o meu Backlog</a>
      </div>
    );
      ...
      <div className="min-h-screen pb-20" style={{ background: V.bg, color: V.text }}>
        <div className="pt-12 pb-8 px-4 text-center border-b" style={{ borderColor: V.border, background: V.card }}>
          <div className="w-28 h-28 mx-auto rounded-3xl overflow-hidden mb-4 shadow-xl" style={{ border: `3px solid ${V.primary}` }}>
            {profile.photoURL ? <img src={profile.photoURL} alt="Avatar" className="w-full h-full object-cover" /> : (
              <div className="w-full h-full flex items-center justify-center text-4xl font-black text-white" style={{ background: `linear-gradient(135deg, ${V.primary}, ${V.secondary})` }}>{profile.displayName?.charAt(0).toUpperCase() || 'G'}</div>
            )}
          </div>
        ...
        <div className="max-w-3xl mx-auto px-4 mt-8 space-y-8">
          {playing.length > 0 && (
            <div>
              <h2 className="text-sm font-black uppercase tracking-wider mb-4 flex items-center gap-2" style={{ color: V.muted }}><Gamepad2 className="w-4 h-4" /> A Jogar Agora</h2>
              <div className="grid sm:grid-cols-2 gap-3">
                {playing.map(game => (
                  <div key={game.id} className="flex gap-4 p-3 rounded-2xl" style={{ background: V.card, border: `1px solid ${V.border}` }}>
                    {game.imageBase64 ? <img src={game.imageBase64} className="w-16 h-20 rounded-xl object-cover" alt="" /> : (
          ...
          <div>
            <h2 className="text-sm font-black uppercase tracking-wider mb-4 flex items-center gap-2" style={{ color: V.muted }}><Trophy className="w-4 h-4" /> Últimos Zerados</h2>
            {finished.length === 0 ? <p className="text-center py-10 text-sm" style={{ color: V.muted }}>Nenhum jogo concluído ainda.</p> : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {finished.map(game => (
                  <div key={game.id} className="relative rounded-2xl overflow-hidden aspect-[3/4] shadow-lg transition-transform hover:scale-105" style={{ border: `1px solid ${game.isPlatinum ? '#f59e0b' : V.border}` }}>
                    {game.imageBase64 ? <img src={game.imageBase64} className="w-full h-full object-cover" alt="" /> : <div className="w-full h-full flex items-center justify-center" style={{ background: V.card }}><Gamepad2 className="w-8 h-8 opacity-30" /></div>}
  ...
  const renderContent = () => {
    ...
    if (!user) return (
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white flex items-center justify-center p-4">
        <div className="max-w-md w-full text-center z-10">
          ...
          <h1 className="text-5xl font-extrabold mb-4 bg-gradient-to-r from-blue-400 via-cyan-400 to-purple-400 bg-clip-text text-transparent drop-shadow-sm">Game Backlog</h1>
    ...
    // Telas de aba — lazy com Suspense
    if (activeTab === "progress") return <Suspense fallback={<ModalLoader />}><LazyProgressScreen gameHistory={gameHistory} gamesData={gamesData} totalFinishedGames={totalFinishedGames} /></Suspense>;
    if (activeTab === "achievements") return <Suspense fallback={<ModalLoader />}><LazyEnhancedAchievements achievements={achievements} gamesData={gamesData} /></Suspense>;
    if (activeTab === "friends") return <Suspense fallback={<ModalLoader />}><LazyFriendsScreen currentUser={{ ...user, photoURL: user.photoBase64 || user.photoURL }} socialProfile={socialProfile} onOpenChat={handleOpenChat} onViewFriendProfile={handleOpenFriendProfile} /></Suspense>;
    if (activeTab === "profile") return <Suspense fallback={<ModalLoader />}><LazyProfileScreen user={user} handleSignOut={handleSignOut} totalFinishedGames={totalFinishedGames} totalCategories={Object.keys(categoryNames).length} totalAchievements={achievements.length} handleProfileImageUpload={handleProfileImageUpload} gamesData={gamesData} goBack={() => setActiveTab("categories")} onOpenBackup={() => setIsBackupOpen(true)} /></Suspense>;
    ...
    if (isRecommenderOpen) return <Suspense fallback={<ModalLoader />}><LazyGameRecommender onClose={() => setIsRecommenderOpen(false)} onSelectGame={handleSelectRecommendation} /></Suspense>;
  ...
  return (
    <div className="relative min-h-screen bg-gray-900">
      ...
      {/* Modais lazy com Suspense */}
      {showWrapped && <Suspense fallback={<ModalLoader />}><LazyWrappedScreen gamesData={gamesData} onClose={() => setShowWrapped(false)} /></Suspense>}
      {showMissions && <Suspense fallback={<ModalLoader />}><LazyWeeklyMissions gamesData={gamesData} onClose={() => setShowMissions(false)} /></Suspense>}
      {showSurprise && <Suspense fallback={<ModalLoader />}><LazySurpriseMe gamesData={gamesData} onClose={() => setShowSurprise(false)} /></Suspense>}
      ...
      {isBackupOpen && <Suspense fallback={<ModalLoader />}><LazyBackupRestore gamesData={gamesData} achievements={achievements} gameHistory={gameHistory} onRestoreData={handleRestoreData} onClose={() => setIsBackupOpen(false)} /></Suspense>}
