...
const ListHeader = ({
  ...
  <div
    className="sticky top-0 z-50 -mx-4 px-4 py-4 mb-6 shadow-xl border-b"
    style={{
      background: `${V.bg}e6`,
  ...
  >
    <div className="max-w-md mx-auto space-y-4">
      <div className="flex items-center gap-3">
        <button
          onClick={onBack}
          className="group flex-shrink-0 flex items-center gap-2 px-3 py-2 rounded-xl border transition-all duration-300 hover:scale-105"
        ...
        <div className="flex-1 text-center min-w-0">
          <div
            className={`inline-block px-4 py-1.5 rounded-full max-w-full`}
            style={{
        ...
        {showFilters && count > 0 && (
          <button
            ...
            style={{
              background: `linear-gradient(to right, ${V.accent}, ${V.primary})`,
            }}
        ...
        <button
          ...
          className="flex-shrink-0 px-3 py-2 rounded-xl font-bold text-sm text-white shadow-lg transition-all"
          style={{
            background: dragMode ? "#10b981" : V.card,
            color: dragMode ? "#fff" : V.text,
            border: `1px solid ${V.border}`,
          }}
        >
          {dragMode ? "✅ Concluído" : "↕️ Reordenar"}
        </button>
      </div>
      ...
      {showFilters && platinumCount > 0 && (
        <div className="flex justify-center">
          ...
          >
            <Trophy
              className={`w-4 h-4 ${showPlatinumOnly ? "fill-yellow-500 text-yellow-500" : ""}`}
            />
            {showPlatinumOnly
              ? `Platinas (${platinumCount})`
              : `Ver Platinas (${platinumCount})`}
          </button>
        </div>
      )}
...
export default function GameList({
  ...
  const [sortOption, setSortOption] = useState(
    selectedCategory === "zerados" ? "date_desc" : "name_asc",
  );
  ...
  const platinumCount = useMemo(
    () => categoryGames.filter((g) => g.isPlatinum).length,
    [categoryGames],
  );
  ...
  const filteredGames = useMemo(() => {
    ...
    if (selectedCategory === "zerados" && selectedYear !== "all") {
      result = result.filter((game) => {
        ...
        return (
          new Date(game.finishedDate).getFullYear().toString() === selectedYear
        );
      });
    }
    result.sort((a, b) => {
  ...
  const exportStats = useMemo(() => {
    ...
    const avgRating =
      rated.length > 0
        ...
        : "—";
    const totalHours = exportGames.reduce(
      (s, g) => s + (parseInt(g.timeToBeat) || 0),
      0,
    );
      ...
      <div className="max-w-md mx-auto px-4 pb-20">
        <div className="space-y-3">
          {filteredGames.length === 0 ? (
            ...
            /* Componente de Arrasto recebendo OnReorder */
            <DragSortList
              games={filteredGames}
              ...
              onReorder={(newGames) => {
                ...
                const updatedCategoryGames = newGames.map((game, index) => ({
                  ...game,
                  sortOrder: index
                }));

                setGamesData(prevData => {
                  const otherGames = prevData.filter(g => g.status !== selectedCategory);
                  return [...otherGames, ...updatedCategoryGames];
                });
                    ...
                    >
                      <div className="relative flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                          {isFinished && (
                            <Trophy
                              className={`w-4 h-4 ${isPlatinum ? "text-yellow-500 fill-yellow-500" : "text-emerald-500"}`}
                            />
                          )}
                          <ChevronLeft
                            className="w-5 h-5 transition-colors rotate-180"
                            style={{ color: V.muted }}
                          />
                        </div>
                      </div>
          ...
          >
            ...
            >
              {[
                ...
                {
                  label: "Nota Média",
                  value: exportStats.avgRating,
                  color: "#f59e0b",
                },
                {
                  label: "Horas",
                  value: `${exportStats.totalHours}h`,
                  color: V.accent,
                },
                ...(exportStats.platinas > 0
                  ? [
                      {
                        label: "Platinas 🏆",
                        value: exportStats.platinas,
                        color: "#fbbf24",
                      },
                    ]
                  : []),
