...
export default function FriendsScreen({ currentUser, socialProfile, onOpenChat, onViewFriendProfile }) {
  ...
  const handleSearch = async () => {
    ...
    if (!code.includes('#')) {
      toast.error('Use o formato: PALAVRA#1234');
      return;
    }
    setSearching(true);
    ...
    try {
      ...
      if (!found) {
        toast.error('Nenhum jogador encontrado com esse código.');
      } else if (found.uid === currentUser.uid) {
        toast.error('Esse é você mesmo! 😄');
      } else {
                  ...
                  <div className="relative bg-gray-800/60 border border-gray-700/50 rounded-2xl p-4 hover:border-gray-600/70 transition-all">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col gap-1.5">
                        <button
                          onClick={() => onViewFriendProfile(uid, profile)}
